...
class Test_005_Search_Functionality:
    ...
    def test_search_functionality_005(self, setup):
        ...
        # l = [product.text for product in self.listOfProducts] # Use list comprehension for simplicity
        if len(self.listOfProducts) > 1 and any("Mac" in product.text for product in self.listOfProducts):
            assert True
